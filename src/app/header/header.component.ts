...
// this.openWeatherService.getOneCallWeather('48073').subscribe(response )    
    setInterval(() => { //every 2 seconds update the time in the component
      this.today = Date();
    },2000);
    setTimeout(()=> { //delay converting the time to so that functions don't run until we get the time. limits bugs
      this.translateTime([this.openWeatherService.sunrise,this.openWeatherService.sunset]);
    },200);
    setInterval(()=> { //every 5 minutes recalculate sun or moon location based on time.
      this.isItDaytime();
      },60000)
  }

  translateTime(array) { //takes an array of two values, the first being the sunrise timestamp, second being sunset timestamp. Then converts them to normal time syntax.
    ...
    if (i === 0) { //if its first through for loop it's sunrise time, therefore no need to subtract 12.
      this.hours = date.getHours();
    } else {this.hours = date.getHours()-12;}; //subtracts 12 from hour to write it on AM/PM schedule
  ...
  isItDaytime():void { //asks if the current time value is in the daylight hours or moonlight
    let currentHour = new Date().getHours();
    if (currentHour === 0) { //conditional statement used if left running it grabs the next days sunrise and sunset
    ...
    moveSun():void { //finds the current value of time, then finds it's percentage through the day we have come
